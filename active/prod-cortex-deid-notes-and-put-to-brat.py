...
def update_brat_db_status(note_id, hdcpupdatedate, directory_location):
    ...
    common.AIRFLOW_NLP_DB.run(tgt_insert_stmt,
                              parameters=(note_id, job_start_date, directory_location, job_start_date, BRAT_PENDING,
                                          hdcpupdatedate))
    print("note {}, {} has been posted for review on brat at location {}".format(note_id,
                                                                                 hdcpupdatedate,
                                                                                 directory_location))
...
def save_note_to_temp_storage(blobid, hdcpupdatedate, metadata_dict):
    ...
    print("saving note metadata to temp_notes for {}, {}: {}".format(blobid, hdcpupdatedate, metadata_dict))
...
def save_person_info_to_temp_storage(blobid, hdcpupdatedate, patient_data):
    insert_stmt = "INSERT INTO temp_person " \
                  ...
                  "VALUES (%s, %s, %s, %s, %s, %s)"
    print("saving person info to temp_person for {}, {}: {}".format(blobid, hdcpupdatedate, patient_data[0]))
    common.ANNOTATIONS_DB.run(insert_stmt, parameters=(blobid,
                                                       hdcpupdatedate,
                                                       patient_data[0],
                                                       patient_data[1],
    ...
    for hdcpupdatedate in hdcpupdatedates:
        ...
        for id_row in common.AIRFLOW_NLP_DB.get_records(tgt_select_stmt,
            ...
            try:
                ...
                print("DEID API called for note: {}, {}".format(blobid, hdcpupdatedate))
            ...
            common.AIRFLOW_NLP_DB.run(tgt_update_stmt,
                                      parameters=(annotation_status,
                                                  ...
                                                  hdcpupdatedate,
                                                  blobid))
            print("DeID Job for note: {}, {} has been updated to {}".format(blobid, hdcpupdatedate, annotation_status))
...
def split_records_by_review_status(records):
    ...
    print("{} records were split into {} for review and {} without review".format(len(records),
                                                                                  len(records_to_review.keys()),
                                                                                  len(records_without_review.keys())))
