...
'''
REDRIVEABLE_CLAUSE = (
    "(resynth_status = '{scheduled_var}' AND DATE_PART('DAY', NOW() - annotation_creation_date) >= {delta}) "
    " OR resynth_status = '{failure_var}' ".format(scheduled_var=common_variables.JOB_RUNNING,
                                                 delta=(int(common_variables.RESYNTH_REDRIVE_THRESHOLD.days)),
                                                 failure_var=common_variables.JOB_FAILURE))

SQL_FETCH_AF3_REDRIVABLE_JOBS = ("SELECT af3_runs_id "
                                 ", hdcorcablobid"
                                 ", hdcpupdatedate"
                                 ", annotation_creation_date "
                                 "FROM ( "
                                 "   SELECT *, ROW_NUMBER() OVER (PARTITION BY hdcorcablobid, hdcpupdatedate "
                                 "                                ORDER BY annotation_creation_date desc) "
                                 "                                AS ROWNUM "
                                 "    FROM af3_runs_details "
                                 "    WHERE {clause} "
                                 "    ) x "
                                 " WHERE ROWNUM = 1 ".format(clause=REDRIVEABLE_CLAUSE))
...
def check_resynth_tasks(upstream_task, **kwargs):
    ...
    push_jobs = [(run_id, date_stamp, job) for job in redriveable_jobs]
    kwargs['ti'].xcom_push(key=common_variables.REDRIVEABLE_RESYNTH_ID, value=push_jobs)
...
def get_redriveable_records():
    ...
    results = [{"af3_runs_id": result[0],
                "hdcorcablobid": result[1],
                "hdcpupdatedate": result[2],
                "annotation_creation_date": result[3]}
