...
def _update_ner_run_details(run_id, blobid, hdcpupdatedate, state):
    tgt_update_stmt = "UPDATE {table} " \
                      ...
                      "WHERE {run_id} = %s " \
                      "AND hdcpupdatedate = %s and hdcorcablobid in (%s) ".format(table=common_variables.AF5_RUNS_DETAILS,
...
def _update_ner_runs(run_id, state):
    tgt_update_stmt = "UPDATE {table} SET job_end = %s, job_status = %s WHERE {run_id} = %s".format(table=common_variables.AF5_RUNS,
                                                                                                    run_id=common_variables.AF5_RUNS_ID)
